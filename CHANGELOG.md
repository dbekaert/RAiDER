...
### Changed
* [TBD](https://github.com/dbekaert/RAiDER/pull/TBD) - Integrate refractivity by exact quadrature of a shape-preserving (PCHIP) reconstruction rather than the trapezoid rule. Refractivity is convex in height, so the trapezoid rule overestimates the delay; on the coarse 32-level pressure-level grid this reduces the mean ZTD error from 8.7 mm to 1.5 mm, validated against the 145-level native grid at four GNSS station locations across four dates. The native model-level path is unaffected (all rules agree to better than 0.3 mm).
* [TBD](https://github.com/dbekaert/RAiDER/pull/TBD) - Compute hydrostatic refractivity from virtual temperature (`k1 * P / Tv`) for consistency with the compensated wet coefficient `k2' = k2 - k1*Rd/Rv`; the previous `k1 * P / T` form double-counted ~2% of the wet delay.
