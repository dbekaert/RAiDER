...
def makePoints0D(double max_len, cnp.ndarray[double, ndim=1] Rays_SP, cnp.ndarray[double, ndim=1] Rays_SLV, double stepSize):
    '''
...
@cython.boundscheck(False)  # turn off array bounds check
@cython.wraparound(False)   # turn off negative indices ([-1,-1])
def makePoints1D(double max_len, cnp.ndarray[double, ndim=2] Rays_SP, cnp.ndarray[double, ndim=2] Rays_SLV, double stepSize):
    '''
    Fast cython code to create the rays needed for ray-tracing. 
    Inputs: 
      max_len: maximum length of the rays
