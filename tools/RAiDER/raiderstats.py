...
def get_extent(dataset, spacing=1, userbbox=None):
    """ Get the bbox, spacing in deg (by default 1deg), optionally pass user-specified bbox. Output array in WESN degrees """
    extent = [np.floor(min(dataset['Lon'])-(spacing/2)),np.ceil(max(dataset['Lon'])+(spacing/2)),
                np.floor(min(dataset['Lat'])-(spacing/2)),np.ceil(max(dataset['Lat'])+(spacing/2))]
    if userbbox is not None:
        ...
        userbbox_poly=Polygon(np.column_stack((np.array([userbbox[2],userbbox[3],userbbox[3],userbbox[2],userbbox[2]]),
            np.array([userbbox[0],userbbox[0],userbbox[1],userbbox[1],userbbox[0]]))))
        if userbbox_poly.intersects(dfextents_poly):
            extent = [np.floor(userbbox[2]-(spacing/2)),np.ceil(userbbox[-1]+(spacing/2)),
                np.floor(userbbox[0]-(spacing/2)),np.ceil(userbbox[1]+(spacing/2))]
        else:
    ...
    nw = [extent[0]+(spacing/2),extent[-1]-(spacing/2)]
    se = [extent[1]-(spacing/2),extent[2]+(spacing/2)]

    #Store grid dimension [y,x]
    grid_dim=[int((extent[-1]-extent[-2])/spacing),int((extent[1]-extent[0])/spacing)]
...
class variogramAnalysis:
    ...
    def __getSamples__(self, data, Nsamp=None): 
        ''' 
    ...
    def __getXY__(self, x2d, y2d, indpars):
        '''
    ...
    def __getDistances__(self, XY, xy = None):
        '''
    ...
    def __empVario__(self, x, y, data, Nsamp = 1e3):
        '''
        ...
        samples, indpars = self.__getSamples__(data)
        x, y = self.__getXY__(x, y, indpars)
        dists = self.__getDistances__(np.array([[x[:,0], y[:,0]],[x[:,1], y[:,1]]]).T)
        vario = self.__getDistances__(samples[:,0],samples[:,1])

        return dists, vario

    def __binnedVario__(self, hEff, rawVario, xBin = None):
        '''
    ...
    def __fitVario__(self, dists, vario, model=None, x0 = None, Nparm = None,  ub = None):
        '''
    ...
    def createVariograms(self):
        ...
        for i in sorted(list(set(self.df['gridnode']))):
            ...
            for j in sorted(list(set(grid_subset['Date']))):
                ...
                if len(np.array(grid_subset[grid_subset['Date']==j]['gnss_minus_wm']))<self.densitythreshold:
                    ...
                    dists, vario=self.__empVario__(lonarr,latarr,delayarray)
                    dists_binned, vario_binned=self.__binnedVario__(dists, vario)
                    #fit experimental variogram for each time AND grid, model default is exponential
                    res_robust, d_test, v_test = self.__fitVario__(dists_binned, vario_binned, model=self.__exponential__, x0 = None, Nparm = 3)
                    ...
                    if self.verbose:
                        ...
                        self.plot_variogram(i, j.strftime("%Y%m%d"), [self.gridpoints[i][1],self.gridpoints[i][0]], workdir=os.path.join(self.workdir,'variograms/grid%i'%(i)), dists=dists, vario=vario, dists_binned=dists_binned, vario_binned=vario_binned) #in verbose
                        #Plot experimental variogram for this gridnode and timeslice
                        self.plot_variogram(i, j.strftime("%Y%m%d"), [self.gridpoints[i][1],self.gridpoints[i][0]], workdir=os.path.join(self.workdir,'variograms/grid%i'%(i)), d_test=d_test, v_test=v_test, res_robust=res_robust.x, dists_binned=dists_binned, vario_binned=vario_binned) #in verbose
                    #append for plotting
            ...
            if dists_binned_arr!=[]:
                ...
                TOT_res_robust, TOT_d_test, TOT_v_test = self.__fitVario__(dists_binned_arr, vario_binned_arr, model=self.__exponential__, x0 = None, Nparm = 3)
                #Plot empirical variogram for this gridnode and timeslice
                ...
                self.plot_variogram(i, tot_timetag, [self.gridpoints[i][1],self.gridpoints[i][0]], workdir=os.path.join(self.workdir,'variograms/grid%i'%(i)), dists=dists_arr, vario=vario_arr, dists_binned=dists_binned_arr, vario_binned=vario_binned_arr, seasonalinterval=self.seasonalinterval)
                #Plot experimental variogram for this gridnode and timeslice
                self.plot_variogram(i, tot_timetag, [self.gridpoints[i][1],self.gridpoints[i][0]], workdir=os.path.join(self.workdir,'variograms/grid%i'%(i)), d_test=TOT_d_test, v_test=TOT_v_test, res_robust=TOT_res_robust.x, seasonalinterval=self.seasonalinterval)
...
class raider_stats(object):
    ...
    def createDF(self):
        ...
        self.plotbbox, self.grid_dim, self.gridpoints= get_extent(self.df, spacing=self.spacing, userbbox=self.bbox)
...
def main(iargs=None):
    ...
    df_stats=raider_stats(inps.fname, inps.workdir, inps.bbox, inps.spacing, inps.timeinterval, inps.seasonalinterval, inps.stationsongrids, inps.colorpercentile, inps.verbose)
    ...
    if inps.station_distribution:
        ...
        df_stats([unique_points.index.get_level_values('Lon').tolist(),unique_points.index.get_level_values('Lat').tolist()],'station_distribution', workdir=os.path.join(inps.workdir,'figures'))
    #Plot mean delay per station
    if inps.station_delay_mean:
        unique_points=df_stats.df.groupby(['Lon', 'Lat'])['gnss_minus_wm'].mean()
    ...
    if inps.station_delay_stdev:
        unique_points=df_stats.df.groupby(['Lon', 'Lat'])['gnss_minus_wm'].std()
        unique_points.dropna(how='any',inplace=True)
    ...
    if inps.grid_delay_mean:
        unique_points=df_stats.df.groupby(['gridnode'])['gnss_minus_wm'].mean()
        unique_points.dropna(how='any',inplace=True)
    ...
    if inps.grid_delay_stdev:
        unique_points=df_stats.df.groupby(['gridnode'])['gnss_minus_wm'].std()
        unique_points.dropna(how='any',inplace=True)
        gridarr_heatmap=np.array([float(0) if i[0] not in unique_points.index.get_level_values('gridnode').tolist() else unique_points[i[0]] for i in enumerate(df_stats.gridpoints)]).reshape(df_stats.grid_dim)
