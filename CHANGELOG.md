...
* [822](https://github.com/dbekaert/RAiDER/pull/822) - Height datums are now explicit. Heights previously flowed into weather-model sampling unconverted, silently treating every source as geoid; they are now kept in their source datum -- ellipsoidal for GNSS positions, ISCE rasters and the downloaded DEM -- and converted only for sampling. **Behaviour changes for existing inputs:**
  ...
  * `RasterRDR`'s `hgt_file` is treated as ellipsoidal, per the ISCE convention it reads.
  * `download_dem` passes `dst_area_or_point=None` rather than `'Area'`, keeping the source pixel registration; requires `dem_stitcher >= 3.0`, which `environment.yml` pins but `pyproject.toml` does not.
