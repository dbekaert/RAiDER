...
class ExistingWeatherModelTooSmall(Exception):
    def __init__(self) -> None:
        msg = 'The weather model passed does not cover all of the input points; you may need to download a larger area.'
        super().__init__(msg)
...
class TryToKeepGoingError(Exception):
    def __init__(self, date=None) -> None:
        if date is not None:
            msg = f'The weather model does not exist for date {date}, so I will try to use the closest available date.'
        else:
