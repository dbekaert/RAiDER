...
def test_era5t():
    wm = ERA5T()
    wm.fetch(np.array([10, 10.1, 10.2]), np.array([-72, -72, -72]), datetime(2020, 1, 1, 0, 0, 0), TEST_DIR / 'weather_model_tests' / 'test_era5t.nc')


@pytest.mark.xfail(reason='ECMWF API is not working for some reason, need to revisit')
def test_erai():
    wm = ERAI()
    wm.fetch(np.array([10, 10.1, 10.2]), np.array([-72, -72, -72]), datetime(2017, 1, 1, 0, 0, 0), TEST_DIR / 'weather_model_tests' / 'test_erai.nc')
