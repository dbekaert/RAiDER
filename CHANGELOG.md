...
### Fixed
* [810](https://github.com/dbekaert/RAiDER/pull/810) - Fixed `checkContainment_raw` returning `False` for every weather model contained in the world box -- that is, every regional model. The final branch returned a bare `False` instead of testing whether the model covered the requested bounds, so cached raw files always looked unusable and `prepareWeatherModel` re-downloaded data already on disk.
...
* [810](https://github.com/dbekaert/RAiDER/pull/810) - Request all dates in a single CDS API call when pulling ERA-5 model levels, rather than one request per date. Pressure-level data is still fetched one request per date: MARS rejects more than one date per month in a `reanalysis-era5-pressure-levels` request.
  * Read pressure-level data from the `reanalysis-era5-pressure-levels` archive, selectable via the new optional `weather_model_levels` config key (`ml`/`model` or `pl`/`pressure`), also exposed as `calcDelaysGUNW --model-levels` and documented in the example templates.
  * Fixed `_load_pressure_level` axis handling: `z` was flipped on the level axis while `t`/`q` were flipped on the latitude axis, so heights were mirrored relative to the meteorology. Axis order is now normalised by dimension name and all cubes are flipped together.
  * Fixed `HRES.load_weather` calling the non-existent `_load_pressure_levels` (plural), which raised `AttributeError` for every HRES pressure-level run.
  * Send `product_type: reanalysis` with pressure-level requests, which the `reanalysis-era5-pressure-levels` dataset requires.
  * Record the units of `z` in the raw files RAiDER writes, so `_load_pressure_level` no longer has to infer geopotential from geopotential height by magnitude. The old threshold (100,000 m**2 s**-2, about 10 km) misread any request whose levels all sat below that.
  * Add unit tests covering the ECMWF/ERA-5 download, batching, level-type selection and geopotential paths.
  * `raider.py` no longer aborts a multi-date run on the first date with no usable weather model data. Each date that fails is recorded and the run continues; `NoWeatherModelData` is raised after the loop only if *every* requested date failed. A run whose dates all fail therefore still errors, but one bad date in a stack no longer discards the good ones.
  * `raiderDownloadGNSS.py` now rejects a `-b`/`--bounding_box` given in the `[0, 360]` longitude convention before any network access, rather than after downloading the UNR station holdings. The check itself is unchanged -- `filterToBBox` already applied it -- only its timing.
