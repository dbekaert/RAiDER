...
### Fixed
* [822](https://github.com/dbekaert/RAiDER/pull/822) - Fixed a height-datum mismatch that biased GNSS zenith delays. The weather-model cube's z-axis is geoid-referenced (the ERA-5 convention) but GNSS station heights are ellipsoidal, and the two were mixed without conversion, displacing the integration surface by the geoid undulation (15-35 m in CONUS) for a ~5-9 mm positive ZTD bias. Heights are now converted to geoid via EPSG:9518 at the one place that needs it, weather-model sampling; `tropo_delay` reads its two consumers separately so the line-of-sight path keeps the ellipsoidal heights its ECEF geometry requires. If the EGM2008 grid is unavailable both locally and from the PROJ CDN, the conversion is skipped with a warning naming the consequence rather than silently reintroducing the bias. Also corrected docstrings in `geo_to_ht`, `weatherModel._get_heights` and `transformPoints`, and the delay cube's `z` attribute, which described geoid-referenced heights as ellipsoidal.
* [816](https://github.com/dbekaert/RAiDER/pull/816) - Stopped offering Herbie the retired University of Utah Pando mirrors when looking for HRRR data. Herbie tries each source it knows until one has the file, so the dead `pando`/`pando2` hosts only cost time when no source has the file -- but that is exactly what an availability check asks, and both hosts still resolve in DNS while accepting no connections. `check_hrrr_dataset_availability` therefore raised `ConnectTimeout` after ~150 s for an unavailable time instead of returning `False`; it now answers in ~1 s. The source list is filtered from the model's own Herbie template rather than hard-coded, so new mirrors are picked up automatically and each model keeps Herbie's ordering.
...
### Changed
* [822](https://github.com/dbekaert/RAiDER/pull/822) - Height datums are now explicit. Heights previously flowed into weather-model sampling unconverted, silently treating every source as geoid; each source is now kept in its own datum -- ellipsoidal for GNSS positions and ISCE geometry rasters, geoid for GLO-30 DEMs -- and converted only where a consumer needs the other. **Behaviour changes for existing inputs:**
  * An unlabelled `Hgt_m` column is assumed ellipsoidal and converted; a file holding MSL heights must now declare `station_file_crs: 4326` in the config's `aoi_group` (default `4979`). `write_yaml` merges only the top level, so a config with its own `aoi_group` must spell this key out.
  * `StationFile.readZ()` writes an authoritative `Hgt_datum` column when it fills heights from a DEM, so a file RAiDER wrote can't later be misread.
  * `StationFile` takes `crs` (default `4979`); `readZ()` takes `geoid_heights: bool = False`.
  * `LOS.setPoints()`'s third parameter is renamed `heights` -> `ellipsoidal_heights` (breaking for keyword callers).
  * `RasterRDR`'s `hgt_file` is treated as ellipsoidal, per the ISCE convention it reads; a DEM it downloads instead is geoid, so the two branches now convert differently.
  * `download_dem` keeps GLO-30 on its native geoid (`dst_ellipsoidal_height=False`) rather than converting to ellipsoidal, so DEM heights reach weather-model sampling in the datum it already uses, converting nothing.
  * The `pyproj` floor rises to `3.1.0`, which the new height transforms need for `pyproj.network` and `Transformer.to_proj4()`.
* [816](https://github.com/dbekaert/RAiDER/pull/816) - Integrate refractivity by exact quadrature of a shape-preserving (PCHIP) reconstruction rather than the trapezoid rule. Refractivity is convex in height, so the trapezoid rule overestimates the delay; on the coarse 32-level pressure-level grid this reduces the mean absolute ZTD error from 8.9 mm to 2.1 mm, validated against the 145-level native grid on ERA-5 across three climate regimes (596 grid columns over equatorial Brazil, subtropical Mexico and arctic Alaska). The improvement is climate-dependent -- 12x in dry arctic columns, 4x in the subtropics, but only 1.6x in the wet equatorial case, where 32 levels cannot resolve the near-surface water-vapour profile and PCHIP still leaves ~4 mm. The native model-level path is unaffected: at 145 levels both rules agree to 0.32 mm.
* [816](https://github.com/dbekaert/RAiDER/pull/816) - Compute hydrostatic refractivity from virtual temperature (`k1 * P / Tv`) for consistency with the compensated wet coefficient `k2' = k2 - k1*Rd/Rv`; the previous `k1 * P / T` form double-counted ~2% of the wet delay. This lowers the total delay by roughly 2% of the zenith wet delay (a few mm in typical conditions), so absolute delays change for every weather model, not just the hydrostatic/wet split.
* [816](https://github.com/dbekaert/RAiDER/pull/816) - Regenerated the processed weather-model test fixtures so their stored `wet`, `hydro`, `wet_total` and `hydro_total` cubes match the new refractivity and quadrature, and updated the affected golden delay values. Note that the ray-traced path still integrates the trapezoid reconstruction, so ray-traced and zenith outputs now differ by roughly 9 mm on pressure-level models; see [817](https://github.com/dbekaert/RAiDER/issues/817).
